...
summer_reward                 — 暑假统一奖励状态；V3 现金只来自抽奖，除预算/券/真实抽奖历史外，还保存 totalEarned、totalRedeemed、parentRedeemPin 和 redemptionHistory 家长兑换账本
...
### V5.49 奖金账户 + 家长兑换账本（build `20260722a`）

- **奖励页置顶奖金账户**：原“勋章”导航改名“奖励”，页面标题改为“奖励与勋章”；最上方集中显示可兑换、累计获得、已兑换和最近奖金流水，勋章收藏整体下移。
- **永久累计真实奖金**：`summer_reward` 升级为 `rewardModelVersion: 3`；新增 `totalEarned`，每次只按抽奖实际到账 `paid` 累加，不再依赖最多 60 条的历史列表计算终身金额。
- **家长部分/全部兑换**：普通奖励页内支持设置 4 位防误触码、全部兑换或输入整数金额部分兑换；页面明确要求家长先完成红包/转账/现金，再把本次金额记为已兑换。
- **兑换流水与纠错**：新增 `totalRedeemed`、`parentRedeemPin` 和 `redemptionHistory`；抽奖收入与家长兑换混合按时间展示，输码后可撤销错记，原流水保留“已撤销”状态且余额恢复。
- **兼容与防重复**：V2 首次读取时以现存真实抽奖历史初始化累计获得；兑换使用 requestId 防止重复提交，确认时重新校验余额；完整备份继续通过原 `data.localStorage` 快照保存新增字段，不改备份版本。
- **缓存版本号**：`index.html` 静态资源版本号和 `BUILD_ID` 更新为 `20260722a`。
