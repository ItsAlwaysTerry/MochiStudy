...
**已落地（build `20260620f`）**：把外部主家教 prompt `skill/gaokao私教.md` 升级为支持「整张卷子一个对话、按题号一道一道做、每搞定一道题输出一条 MOCHI-RECORD、做一道粘一条」——不再假设一题一对话，也不另加新 prompt（避免 prompt 越堆越多）。设置页该 prompt 的说明同步改写，强调可以一次拍整张卷子、说题号。导入侧无需改动：老网站本就支持单条/多条 MOCHI-RECORD 粘贴导入。

**啃卷子并入主家教（build `20260620h`）**：按「学生只设一个 Gem、长期用、减少 prompt 选择」的诉求，把 `gaokao啃卷子.md` 整体并进 `gaokao私教.md`——做题就一个智能体，开场二选一：发一道题直接做 / 拍整张卷选「帮我排优先级」（读图列题→排除已会→考频×提分两维度排序→逐题带）。后半段逐题脚手架 + 每题出记录两用法共用，记录加 `学习来源：[新学/错题]`。删除 `skill/gaokao啃卷子.md` 与设置页其独立入口，做题入口只剩一个；AI 指南从「五个」改「四个」。study_log/导入/备份不变。

### 引入《试题调研》辅导书学习体系（build `20260627a`）

背景：站外 Gemini 一题题讲解=零碎知识点，缺宏观高考方向（考频/题型/解法框架）。引入天星教育《试题调研》第一阶段 18 本（数/物/化 × 辑1/2/3/4/5/7）当"专业研究过的路径"。书定方向、Gemini 当老师、纸质地图打勾、MochiStudy 当动力。**核心架构原则：书的正文（模型/解法/例题）靠拍照进 Gemini（原生读图），不做全书 OCR/RAG；只把"目录浓缩索引"喂 Gem 做交叉引用。**

- **数据/底料**：`docs/试题调研-全18本索引.md`——18 本浓缩索引（按科目组织，辑1-4 超重点/考点、辑5 模型、辑7 招名+高考考频），剥页码、修 OCR 错字、残缺处标"未编造待补"。是下面三者的共同底料。
- **三合一 Gem = 生产 `skill/gaokao私教.md`（已合并，build `20260628a`）**：用户选 B，把"啃辅导书 / 做题定位 / 整卷排序"三模式合进唯一的做题 Gem。**开头第〇步先判断**学生发来的是「辅导书页（有印刷的超重点/快招/模型/例题讲解结构）」还是「自己作业/卷子的题」，走不同流程、记不同来源——这是合并的命门（用户特别要求）。① 辅导书页→五步法（定位补基础→读例题不读解法→带啃书解法贴招名→合书复现→出记录）+ 两规则（例题取舍只复现一道、超重点过完才串招），`学习来源：辅导书`；② 整卷→读图列题(带辅导书定位)+排优先级(考频维度引辑7)+逐题带做，`学习来源：错题`；③ 单题→先一行定位钉回辅导书+逐题带做，`学习来源：新学/错题`。**做题定位 5 护栏**（不乱塞辑、覆盖不到认怂、考频照抄不编、内容线+招线都给、逐题不漏）已用 `docs/做题Gem-测试卷.md`（10 题埋 4 陷阱）压测全过；保留原私教全部深度（脚手架/话术/科目/记录情况A·B）；索引全 18 本内联到 prompt 末尾，单文件即可粘 Gem。
- **被取代的中间件（保留备查，已不用）**：`docs/做题Gem-完整粘贴版.md`、`docs/做题Gem-指令-测试版.md`（测试件）、`skill/gaokao辅导书.md`（独立啃书版，逻辑已并入三合一）。现在学生只设**一个 Gem**＝三合一 `gaokao私教.md`。
...
### 移除《试题调研》辅导书绑定，做题私教回归"第一性原理精简版"（build `20260717t`）

背景：真实用户反馈 `skill/gaokao私教.md`（三合一：啃辅导书五步法 + 做题定位 + 整卷排序，绑定《试题调研》18 本索引）**把他当弱智**——一道立体几何截面题被拆成十几轮小到侮辱人的问答，每轮还重念一大堆点的字母，答一个芝麻大的点又要等回复，繁琐到炸（见 `docs/Gemini-_49.md` 聊天记录，用户当场喊停、Gemini 切成"直接给骨架"才顺）。且辅导书绑定用户已不用。按用户"从第一性原理重设、精简版（只带做题+出记录）、连带清理辅导书周边"的决定重做：

- **`skill/gaokao私教.md` 整体重写为精简做题家教。** 核心新增**顶部第一铁律「粒度自适应」**：默认给"解题骨架/关键几步"让学生自己走，只有他真卡住才拆那一步、不整题重来；学生随时可喊"给骨架就行/太碎了/直接讲这步"立刻照办；每条消息短、别每轮重念所有字母、让他看自己拍的图、少用抽象 θ/v₀ 多用具体数；prompt 里直接写进反面教材（别连问十轮"这两点在不在同一面"）。保留护自信铁律（不甩完整答案/只夸具体/不说错了/每题闭环）、科目起手式、收尾复述套路+3步标准模型（变式题降为可选）、MOCHI-RECORD 格式与预设知识点。
- **删掉**：第〇步判断辅导书页 vs 题、模式一啃书五步法、钉回辅导书的定位+5护栏、整卷排序打分表、全部辑1-7索引（约 110 行）。`学习来源` 只剩 新学/错题（不再产出"辅导书"；`app.js` `normalizeSource` 与 `knowledgeMap.js` `sourceDisplayInfo` 的 textbook 分支保留给旧卡兼容）；`题型标签` 改为自由的 题型·方法名（如 立体几何·截面·延长相交法），不再写辑号。一次多题仍处理，就一道一道带、做一道出一条，不带排序。
- **设置页 AI 指南**第一个 prompt 的描述/使用方法同步改成精简做题流程（去掉"拍整张卷排优先级啃卷子"字样），仍是"四个 Prompt"。
- **清理辅导书周边文件**（git 历史保留，可恢复）：删 `skill/gaokao私教-Gem精简版.md`、`skill/gaokao辅导书.md`、`skill/做题Gem-完整粘贴版.md`、`docs/试题调研-全18本索引.md`、`docs/试题调研-纸质地图.html`、`docs/做题Gem-测试卷.md`。保留另三个 prompt（复习/综合测验/重学）。
- 缓存版本号 `20260717s → 20260717t`。
