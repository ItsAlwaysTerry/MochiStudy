...
### 复习模块 V1（modules/reviewEngine.js + modules/reviewPage.js）

- 新增 `#review` 复习页，本地规则计算待复习优先级
- 按知识点聚合，显示低星次数、上次复习结果、下次建议日期
- 点"开始复习"自动复制单知识点 AI 复习素材包
- 只有回填 MOCHI-RECORD 并成功导入才算完成复习
- "待巩固 / 近期稳定"进入折叠区，避免连续压力
- 导入成功后提示"去更新核心摘要"，可跳转档案并自动打开编辑

### 摘要可编辑 V1.1（modules/knowledgeMap.js）

- 学习档案知识点展开后先显示核心摘要卡
- 摘要可人工校正（主要卡点 / 核心突破 / 复习备注），写入 `study_node_summary`
- 有人工摘要时显示"已校正"，可随时恢复自动摘要
- 单知识点复习包优先携带人工摘要
- 首页新增"今日复习"极简卡片，点击直接开始复习

### UX 减压与引导 V1.2

- **Bug 修复**：整理模式拖拽排序后历史卡片会意外收起，现已修复（STATE.historyExpanded 跟踪展开状态）
- **首页导入框提前**：右侧列顺序改为 农场→导入框→今日目标→今日复习，导入是最高频操作
- **首页空状态引导**：无记录时显示三步上手引导卡，替代无意义的空数据展示
- **复习页减压**：待处理列表超过 4 条时默认折叠，避免长列表带来焦虑
- **整理模式提示**：开启后列表上方出现状态条，tooltip 说明按钮功能
- **移动导航优化**：底部导航改为 首页/复习/档案/日历/设置，复习提前到第二位
