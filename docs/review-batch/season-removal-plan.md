...
3. 旧 `current_season`/`season_archives` 数据：只停用不删除（推荐），确认？
   1. 同意，但做这个赛季的初衷就是让学生有那个紧迫感的那个DDL，因为它原来是没有计划的，只是在随机的去问一些平时不会的题目，然后就很懒散，就是很松散，没有一个紧迫感，没有一个结构，也没有计划。所以我搞这个赛季出来，只是想人为创造地点，然后给他切割学习时间，但是发现都不实用。但是现在这部分功能已经几乎没必要了，因为他已经有了明确的学习计划，就是我们现在给他做的那些一轮复习，所以计划DDL这方面已经完成了。然后这个数据啊，这个赛季唯一有用的就是记录数据，把他学习的时间啊，还有题目之类的东西可视化，只要保留这个东西，然后这个赛季就可以完全删掉了
